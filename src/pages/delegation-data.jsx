                        ...
                        filteredTasks.map((task) => (
                            <div key={task._id} className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Timestamp</div>
                                        <div className="text-sm text-gray-500">{task.Timestamp || "—"}</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Task ID</div>
                                        <div className="text-sm font-medium text-gray-900">{task['Task ID'] || "—"}</div>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-3 mb-3">
                                    <div>
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Department</div>
                                        <div className="text-sm text-gray-500">{task.Department || "—"}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Given By</div>
                                        <div className="text-sm text-gray-500">{task['Given By'] || "—"}</div>
                                    </div>
                                </div>
                                <div className="mb-3">
                                ...
                                <div className="mb-3">
                                    <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Task Description</div>
                                    <div className="text-sm text-gray-500 break-words">{task['Task Description'] || "—"}</div>
                                </div>
                                <div className="flex justify-between items-start mb-3">
                                    <div className="bg-yellow-50 p-2 rounded flex-1 mr-2">
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Task Start Date</div>
                                        <div className="text-sm text-gray-500">{task['Task Start Date'] || "—"}</div>
                                    </div>
                                    <div className="flex-1 ml-2">
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Frequency</div>
                                        <span className={`inline-block px-2 py-1 rounded-full text-xs mt-1 ${task.Freq === 'Daily' ? 'bg-blue-100 text-blue-800' :
                                            task.Freq === 'Weekly' ? 'bg-green-100 text-green-800' :
                                                task.Freq === 'Monthly' ? 'bg-purple-100 text-purple-800' :
                                                    'bg-gray-100 text-gray-800'
                                            }`}>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Enable Reminders</div>
                                        <div className="text-sm text-gray-500">{task['Enable Reminders'] || "—"}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Require Attachment</div>
                                        <div className="text-sm text-gray-500">{task['Require Attachment'] || "—"}</div>
                                    </div>
                                </div>
                            </div>
                        ))
